...
// A spoken message from the children.
//
// Telegram only accepts a real voice note as OGG/Opus. A tablet browser may
// record that, or it may only manage WebM/Opus — so WebM is sent as an audio
// file instead. It arrives either way; a voice note simply plays inline while
// an audio file needs a tap.
export async function sendVoice(bytes, mime, caption) {
  const c = creds();
  if (!c) throw new Error("Telegram is not configured");

  const ogg = /ogg/i.test(mime || "");
  const method = ogg ? "sendVoice" : "sendAudio";
  const field = ogg ? "voice" : "audio";
  const name = ogg ? "message.ogg" : "message.webm";

  const form = new FormData();
  form.set("chat_id", c.chatId);
  if (caption) form.set("caption", caption);
  form.set(field, new Blob([bytes], { type: mime || "application/octet-stream" }), name);
  return call(method, form);
}
