...
// --- call ring -------------------------------------------------------------
//
// Deliberately unlike the SOS siren. A call is not an emergency: this is a
// soft two-note that repeats every few seconds, enough to be heard from
// another room but not enough to frighten anyone. It stops the instant the
// call is answered or dismissed.

let ring = null;

export function startRing() {
  if (ring) return;
  const chime = () => playSound({ notes: [784, 1046.5], vol: 0.14, cycles: 1, step: 0.22, dur: 0.5 });
  chime();
  ring = setInterval(chime, 4000);
}
