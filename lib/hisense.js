...
// A cloud round trip is ~1s, but a write also waits for the change to show up
// in a later read, so the worst case is several of them. This must stay above
// the service's own polling budget or writes fail with a timeout instead of
// completing.
const TIMEOUT = 20000;
...
export async function setUnit(puid, patch) {
  ...
  const unit = toUnit({ puid, status: res.status, type: "009" });
  // If the cloud has not caught up yet, keep showing what was asked for
  // instead of the stale reading, which would flip the control back.
  return res.pending ? { ...unit, ...patch, pending: true } : unit;
}
