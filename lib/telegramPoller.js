  ...
  g.__tgDrain = (async () => {
    try {
      ...
      if (messages.length) {
        ...
        // Also not awaited: downloading a photograph takes far longer than a
        // poll should, and nothing on screen is waiting for it.
        keepPhotos(messages).catch((err) =>
          console.log("[photos] failed:", err.message),
        );
      }
    } finally {
      g.__tgDrain = null;
