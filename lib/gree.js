...
const PORT = 7000;
// The unit answers in milliseconds when it is on the network. A long timeout
// only buys a longer wait when it is off or we are away from home, and the
// dashboard holds the whole AC row until this resolves.
const TIMEOUT = 1200;
...
// app rebound it), which a rebind fixes. But if the unit simply did not answer
// there is nothing to rebind to, and retrying just doubles the wait — so a
